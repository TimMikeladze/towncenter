...
class AliasTest(unittest.TestCase):
    def test_units_without_a_tree_node_borrow_a_companion_icon(self):
        ...
        self.assertEqual(sync.ICON_ALIASES["Unit"][42], 331)
        self.assertEqual(sync.ICON_ALIASES["Unit"][1252], 1225)

    def test_the_nodeless_town_center_borrows_the_other_town_center_icon(self):
        self.assertEqual(sync.ICON_ALIASES["Building"][71], 109)

    def test_entities_with_no_icon_anywhere_upstream_are_not_aliased(self):
        # Cartography, Tracking and the Aztec Raider appear in no civ tree, so
        # upstream ships no picture for them and there is nothing to borrow.
        self.assertNotIn(19, sync.ICON_ALIASES["Tech"])
        self.assertNotIn(90, sync.ICON_ALIASES["Tech"])
        self.assertNotIn(1570, sync.ICON_ALIASES["Unit"])
