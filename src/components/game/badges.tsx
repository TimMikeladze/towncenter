...
/**
 * The game files carry no unit or building category, so these are this app's
 * own grouping, derived from the training building and armour classes. The
 * tooltip says so wherever a category chip appears.
 */
export function TypeBadge({ type, className }: { type: string; className?: string }) {
  return (
    <Chip tone={TYPE_TONE[type]} className={className} title="Category grouped by this site, not a game data field">
      {type}
    </Chip>
  )
}
