...
import { getAllUnits, getUnitById } from "@/lib/data"
import { Badge } from "@/components/ui/badge"

export default function CountersPage() {
  const allUnits = getAllUnits()
  const [selectedUnit, setSelectedUnit] = useState(allUnits[6].id) // Default to Knight

  const currentUnit = getUnitById(selectedUnit)

  if (!currentUnit) return null

  const counters = currentUnit.counters.map((id) => getUnitById(id)).filter((u): u is NonNullable<typeof u> => u !== undefined)
  const goodAgainst = currentUnit.goodAgainst.map((id) => getUnitById(id)).filter((u): u is NonNullable<typeof u> => u !== undefined)
